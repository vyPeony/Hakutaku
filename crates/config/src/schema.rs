...
pub struct DataSourceConfig {
    ...
    /// `\` へ統一済み。
    ///
    /// # glob 記号は解釈しない（Issue #39 で明文化）
    ///
    /// この値は常に**そのままのパス**として扱う。`*` や `?` を書いても
    /// [`LogProfileConfig::path_pattern`] のようなパターンとしては解釈せず、
    /// 起動時検証も通過する（ADR-0005 の判定はパス文字列の先頭部分の形式だけを
    /// 見るため）。Windows のファイル名に `*`・`?` は使えないため、結果は
    /// 「その名前の場所が見つからない」という実行時の失敗になる。
    pub path: PathBuf,
}
