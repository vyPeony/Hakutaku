...
//! コメントを参照）。
//!
//! `str::to_uppercase` は1文字を複数文字へ変える場合がある（例: `ß` → `SS`）。
//! 照合は変換**後**の文字列に対して行うため、そうした文字を含むパスでは `?`
//! （1文字一致）が消費する文字数が元の文字列での直感と食い違い得る
//! （Issue #39 で明文化した近似の帰結。実運用のログのパスは ASCII と日本語が
//! 主であり、この差が問題になる場面は想定していない）。
...
/// # 最悪計算量（Issue #39 で明文化）
///
/// `**` は消費する階層数を総当たりするため、`**` を k 個含むパターンでは最悪
/// O(n^k)（n は対象パスの階層数）になる。実運用で書かれるパターンの `**` は
/// 高々1〜2個、階層数も十数程度であり、この総当たりで問題は起きない。記憶化
/// （動的計画法）で O(n×m) にできるが、実測で問題が出ていない段階で実装を
/// 複雑にしないという判断であり、極端なパターンでは照合が遅くなり得る。
