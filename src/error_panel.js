 ...
 *
 * 手動でのログ解析プロファイル選択と日時書式選択は、生表示へ退避した対象の
 * タブを表示している間、右ペインのツールバーに常設される「選んで再解析」
 * 操作（`src/shell.js` の `buildReparseControl`。配置は Issue #97 の左ペイン
 * 再設計の裁定）から行う。この通知自体は、その操作へ気づく前の
 * 最初の手がかりとして「生表示になったこと」を伝えるためのものである。
 *
  ...
  requestNotice({
    ...
    buildBody: () => {
      ...
      text.textContent =
        ...
        "全行を日時未解析のまま表示しています（LOG-022）。この対象のタブを表示中に" +
        "上部ツールバーへ現れる「選んで再解析」から、ログ解析プロファイルと日時書式の" +
        "いずれか、または両方を指定して開き直すことができます。設定ファイルに" +
        "書いていないファイルでも、日時書式（LOG-DT-001〜006）を選べばその書式で" +
        "解析できます。";
      return [text];
    },
  });
