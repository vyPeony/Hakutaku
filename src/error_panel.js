 ...
 *
 * 手動でのログ解析プロファイル選択（P07-2）と日時書式選択は、生表示へ退避
 * した対象のタブを表示している間、右ペインのツールバーに常設される
 * 「選んで再解析」操作（`src/shell.js` の `buildReparseControl`。左ペイン
 * 再設計で行内から移設）から行う。この通知自体は、その操作へ気づく前の
