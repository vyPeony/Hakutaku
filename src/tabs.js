 ...
 *
 * `LOG-028`（明示的な再読み込み）で使う。対象一覧の行で「再読み込み」を
 * 押した対象は、必ずしも現在アクティブなタブとは限らない（背景の行にも
 * 押せる）。バックグラウンドの対象を再読み込みしただけで利用者の視点が
 * 奪われるのは驚きが大きいため、フォーカスは変えずタブの中身だけを最新化
 * する（`src/shell.js` の `handleReloadTargetClick` 参照）。
 *
