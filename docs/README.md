...
| 検証 | [要件と充足根拠の対応表](verification/traceability.md) | 検証記録に現れない要件、コード上に ID 参照がない要件の充足根拠と、未検証・検証不能の所在 |
...
### 検証記録を草案のまま運用する理由と、その引用

検証記録（[手動での動作確認手順](verification/manual-check.md)、[段階0検証記録](verification/stage0-results.md)）は、実施のたびに節を追記し続けるため、草案（`Draft`）のまま運用します。全体が完成して採用済み（`Accepted`）になる性質の文書ではありません。

この草案は「これから決めることの作業仮説」ではなく、**すでに起きた事実の記録**です。したがって次のように扱います。

- **節単位の事実記録（実測値、確認結果、実施できなかった項目とその理由）は、採用済み文書からも確定的な根拠として引用できます。** 文書全体の状態が草案であることは、記録済みの事実の確からしさを下げません
- 引用する側は、文書名だけでなく**節番号**で参照します（例: 段階0検証記録の 10.1 節）。節は追記で増えるため、文書全体を指す引用は時間が経つと何を指すか分からなくなります
- 記録の更新は**追記**で行い、既存の節の書き換えでは行いません。後から分かったことは「後日の追記」として、当時の記録を残したまま加えます
- 一方、検証記録の中の**推測・未確定と明記された記述**（原因の推定、残存リスク、今後の見込み）は事実記録ではありません。これらを確定仕様の根拠にはしません
