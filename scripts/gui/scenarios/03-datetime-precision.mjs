...
// 同じ日時が2回並んで見えていた）。解析済み日時を利用者へ提示する経路は、
// ツールバーのコピー列「日時」（ADR-0009）に一本化されている。
//
// そのコピーの供給元は Rust 側であり、フロントエンドが受け取る日時の表示文字列は
// `fetch_log_range` の応答（`src-tauri/src/log_view.rs` の `LogItemDto.timestamp`）
// で決まる。ここが桁を落とす／引き伸ばすと、コピー結果も同じように壊れる。
// そこでこのシナリオは、フロントエンドが使うのと同じコマンド・同じ引数で
// IPC 境界を直接叩き、返る表示文字列の精度が保たれていることを表明する。
// 日時の解析器（Rust 側）込みで検査するという価値は、DOM を見ていた頃と変わらない。
//
